...
class EnterpriseRepository:
    def create(self, data):
        db = get_db()
        enterprise = Enterprise(name=data.name, email=data.email, phoneNumber=data.phoneNumber, id_login=data.id_login, address=data.address, city=data.city, state=data.state, cep=data.cep, number=data.number, complement=data.complement, cnpj=data.cnpj, status=data.status)
